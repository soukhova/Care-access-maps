...
load(file = "data/HAM_census_21.rda")
...
Community_Boundaries <- Community_Boundaries |> mutate(COMMUNITY_ = ifelse(COMMUNITY_ == "Hamilton", "Hamilton Centre", COMMUNITY_))
...
Escpartment_Boundary_HAM <- Escpartment_Boundary |> st_intersection(Community_Boundaries |> filter(COMMUNITY_ == "Hamilton Centre")) 
care_dest <- care_dest |> st_set_crs(st_crs(Community_Boundaries))
```

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
hydro_p_LakeOntario <- st_read(paste0(here::here(),"/data/Boundaries/hydro_p_LakeOntario.shp")) |> st_transform(crs=4326)
ham_bay <- st_read(paste0(here::here(),"/data/Boundaries/Waterbodies.shp")) |> st_transform(crs=4326) |> filter(FEATURE_TY == "Lake")
```



```{r, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
#plotting map to export as a figure for papa
#tmap_mode("plot")
...
p_pop <- 
  ...
  tm_polygons("Population",
              palette = "Greens",
              style = "quantile", n=4, alpha=0.6,
              border.alpha = 0.1, lwd=0.01,
              title = "Population (Quantiles)",
              legend.is.portrait = TRUE) +
  tm_shape(Community_Boundaries, is.master = TRUE) + 
  tm_polygons("COMMUNITY_", bord.col="black", alpha = 0, border.alpha = 1, lwd=2, legend.show=FALSE) + 
  tm_shape(Escpartment_Boundary) + tm_lines(col = "orange", lty = "dashed", lwd=2, legend.show=TRUE)+
  tm_shape(Community_Boundaries) + tm_text("COMMUNITY_", size=1.1) + 
  tm_compass(position = c("left","top"))+
  tm_scale_bar(position = c("left","bottom"))+
  tm_add_legend(type = c("line"), labels = c("Community boundaries", "Escarpment"), col = c("black", "orange"), lwd = c(2, 2), lty = c("solid", "dashed"))+
  tm_layout(bg.color = "grey", legend.position = c("right","top"), legend.text.size = 1)

tmap_save(p_pop, "figures/p_pop.png")
```
```{r, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
#plotting map to export as a figure for paper
ttm_care_TRANSIT_all <- rbind(ttm_care_TRANSIT_parcel1, ttm_care_TRANSIT_parcel2,ttm_care_TRANSIT_parcel3,ttm_care_TRANSIT_parcel4)
rm(ttm_care_TRANSIT_parcel1, ttm_care_TRANSIT_parcel2,ttm_care_TRANSIT_parcel3,ttm_care_TRANSIT_parcel4)

ggplot(ttm_care_TRANSIT_all, aes(x=travel_time_p50)) + 
         geom_density(bw=0.5) #not needed I think!

ttm_care_TRANSIT_all |> group_by(from_id) |> summarise(mean_tt = mean(travel_time_p50))  |> summary()
```
...
```{r, message=FALSE, warning=FALSE, echo=FALSE}
sf_use_s2(FALSE)
Community_Boundaries_HAM <- Community_Boundaries |> filter(COMMUNITY_ == "Hamilton Centre") 
...
The closer the points are to the vertical line -- the more spatially autocorrelated the dataset is. In the following, we spatially visualise the statistically significant results where the top right quadrant of the plot above are (high-high) and the bottom left quadrant are (low-low) values. We also add in the Escarpment boundary within Hamilton Central: 
...
tm_shape(access_15_parcel_DA_HAM) + tm_fill(col = "quadrant", title = "", breaks = c(1, 2, 3, 4, 5, 6), 
  ...
  tm_shape(Escpartment_Boundary) + tm_lines(col = "forestgreen", lwd=2)+
           ...
           filter(
            ...
            #lower hamilton central - low values DONE (Q1) 35250265
              ...
              # mountain hamilton central - high values 
               ...
               # POTENTIAL mountain hamilton central - low values 
                    GeoUID == 35250583| GeoUID == 35250992| GeoUID == 35250989 | GeoUID == 35250582 | GeoUID == 35250581 | GeoUID == 35250578
            )
...
- LOWER - HAMILTON CENTRAL:
  - high (**35250434** (the highest value), 35250424, 35250422, 35250427, *35250854*) + ~X destinations
  - low (**35251044** (the lowest value), 35250238, 35250248, 35250320,35250319,35250281 ) + ~X destinations
  OOOOORR
  - low (**35250266** (the lowest value), 35250265, 35250264, 35250263, 35250857, 35250200) + ~X destinations
  
- ON THE MOUNTAIN - HAMILTON CENTRAL:
  - high (**35250379** (the highest value),35250442, 35250378, 35250443, *35250377*, *35250376*) + ~X destinations 
  - POTENTIAL SELECTION low ( **35250583** (the lowest value), 35250992, 35250989, 35250582,  35250581, 35250578) + ~X destinations
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
tmap_mode("plot")
care_dest_HAM_renamed <- care_dest_HAM |> 
  mutate(Care_Category = case_when(Care_Category == "Child_Centric" ~
                                                                   "Child-centric",
                                                                   Care_Category == "Elder_Centric" ~
                                                                   "Elder-centric",
                                                                   Care_Category == "Grocery_Centric"~
                                                                   "Grocery-centric",
                                                                   Care_Category == "Health_Centric"~
                                                                   "Health-centric",
                                                                   Care_Category == "Errand_Centric"~
                                                                   "Errand-centric")) |>
  filter(Care_Category != "Elder-centric")

p_access_30_selected <-
  tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_30_parcel_DA) +
  tm_polygons("med_copp_isc",
              palette = c("darkred", "orange", "yellow", "green"),
              alpha=0.6,
              breaks = c(2, 145, 257.8, 408.2, 807),
              border.alpha = 0.2,
              lwd=0.5,
              title = "Median accessibility \n(Hamilton Centre quantiles)") +
  tm_shape(Community_Boundaries_HAM, is.master = TRUE) + tm_borders("grey20", lwd=1.5) + 
  tm_shape(Community_Boundaries) + tm_borders("grey20", lwd=2) + 
  tm_shape(Escpartment_Boundary) + tm_lines(col = "white", lty = "dashed", lwd=1.5, legend.show=TRUE)+
  tm_shape(access_30_parcel_DA_HAM_selected) + tm_borders(col="white", lwd=1.5)+
  tm_shape(care_dest_HAM_renamed) +
  tm_dots(col = "Care_Category", palette = c("magenta","#0b0ba1","#007a99", "cyan"), size = 0.01, title ="Care destinations")+
  tm_compass(position = c("right","top"))+
  tm_scale_bar(position = c("left","bottom"))+
  tm_add_legend(type = c("line"), labels = c("Community boundaries", "Escarpment", "Audit DA"), 
                col=c("black", "grey70", "grey90"), lwd = c(2, 2), lty = c("solid", "dashed"))+
  tm_layout(bg.color = "grey", legend.outside = TRUE, legend.outside.position = c("right","top"))

p_access_30_selected
tmap_save(p_access_30_selected, "figures/p_access_30_selected.png")
```
...
p_clusted_30_selected <- 
  tm_shape(ham_bay, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0) +
  tm_shape(hydro_p_LakeOntario, bbox=Community_Boundaries) + tm_polygons(col="skyblue", border.alpha = 0)+
  tm_shape(access_30_parcel_DA_HAM) +
  tm_fill(col = "quadrant", title = "Spatial autocorrelated\nclusters", 
            breaks = c(1, 2, 3, 4, 5, 6), 
            palette =  c("red", "blue", "lightpink", "skyblue2", "white"),
            labels = c("High-High", "Low-Low", 
                       "High-Low","Low-High", 
                       "Non-significant")) +
  tm_shape(Community_Boundaries_HAM, is.master = TRUE) + tm_borders("grey20", lwd=2) + 
  tm_shape(Escpartment_Boundary) + tm_lines(col = "orange", lty = "dashed", lwd=2, legend.show=TRUE)+
  tm_shape(access_30_parcel_DA_HAM_selected) + tm_borders(col="orange", lwd=2)+
  tm_compass(position = c("right","top"))+
  tm_scale_bar(position = c("left","bottom"))+
  tm_add_legend(type = c("line"), labels = c("Community boundaries", "Escarpment", "Audit DA"), 
                col=c("black", "orange", "orange"), lwd = c(2, 2), lty = c("solid", "dashed"))+
  tm_layout(bg.color = "grey", legend.outside = TRUE, legend.outside.position = c("right","top"), legend.title.size = 0.9)

p_clusted_30_selected
tmap_save(p_clusted_30_selected, "figures/p_clusted_30_selected.png")
...
Now save the access files as shapefiles for use in other GIS software:
```{r}
st_write(HAM_census_21, "data/shp-of-care-dest-and-acc/HAM_census_21.shp")
st_write(care_dest, "data/shp-of-care-dest-and-acc/care_dest.shp")

st_write(access_30_parcel_DA, "data/shp-of-care-dest-and-acc/access_30_parcel_DA.shp")
st_write(access_15_parcel_DA, "data/shp-of-care-dest-and-acc/access_15_parcel_DA.shp")
```
